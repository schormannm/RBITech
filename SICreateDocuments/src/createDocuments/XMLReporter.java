...
public class XMLReporter
	...
	public static void process_files(String filename, String template) throws Exception
		...
		if (f.exists() && !f.isDirectory())
			...
			/*
			 * // ================================================================================ // How about we try to output
			 * this thing to a PDF //
			 * 
			 * // Refresh the values of DOCPROPERTY fields FieldUpdater updater = new FieldUpdater(wordMLPackage); //
			 * --updater.update(true);
			 * 
			 * // Set up font mapper (optional) // -- Mapper fontMapper = new IdentityPlusMapper(); // --
			 * wordMLPackage.setFontMapper(fontMapper);
			 * 
			 * // -- PhysicalFont font = PhysicalFonts.get("Arial Unicode MS");
			 * 
			 * // make sure this is in your regex (if any)!!! // if (font!=null) { // fontMapper.put("Times New Roman", font); //
			 * fontMapper.put("Arial", font); // } // fontMapper.put("Libian SC Regular", PhysicalFonts.get("SimSun"));
			 * 
			 * // FO exporter setup (required) // .. the FOSettings object FOSettings foSettings = Docx4J.createFOSettings();
			 * 
			 * foSettings.setWmlPackage(wordMLPackage);
			 * 
			 * // Document format: // The default implementation of the FORenderer that uses Apache Fop will output // a PDF
			 * document if nothing is passed via // foSettings.setApacheFopMime(apacheFopMime) // apacheFopMime can be any of the
			 * output formats defined in org.apache.fop.apps.MimeConstants eg // org.apache.fop.apps.MimeConstants.MIME_FOP_IF or //
			 * FOSettings.INTERNAL_FO_MIME if you want the fo document as the result. //
			 * foSettings.setApacheFopMime(FOSettings.INTERNAL_FO_MIME);
			 * 
			 * String output_PDF = BASE_PATH + getFilenameWithoutExtension(filename) + ".pdf"; // exporter writes to an
			 * OutputStream. OutputStream os = new java.io.FileOutputStream(output_PDF);
			 * 
			 * // Specify whether PDF export uses XSLT or not to create the FO // (XSLT takes longer, but is more complete).
			 * 
			 * // Don't care what type of exporter you use Docx4J.toFO(foSettings, os, Docx4J.FLAG_EXPORT_PREFER_XSL);
			 * 
			 * // Prefer the exporter, that uses a xsl transformation // Docx4J.toFO(foSettings, os, Docx4J.FLAG_EXPORT_PREFER_XSL);
			 * 
			 * // Prefer the exporter, that doesn't use a xsl transformation (= uses a visitor) // .. faster, but not yet at feature
			 * parity // Docx4J.toFO(foSettings, os, Docx4J.FLAG_EXPORT_PREFER_NONXSL);
			 * 
			 ...
			 * 
			 * // This would also do it, via finalize() methods updater = null; // -- foSettings = null;
			 */
